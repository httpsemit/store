                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    <div>
                        ...
                        <input
                            ...
                            className="w-full px-4 py-3 bg-gray-50 border border-transparent rounded-xl focus:bg-white focus:border-indigo-500 transition-all font-bold text-gray-900"
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            className="w-full px-4 py-3 bg-gray-50 border border-transparent rounded-xl focus:bg-white focus:border-indigo-500 transition-all font-medium text-gray-700 h-20 resize-none"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input
                                ...
                                className="w-full px-4 py-3 bg-gray-50 border border-transparent rounded-xl focus:bg-white focus:border-indigo-500 transition-all font-bold"
                                min="0"
                            />
                        </div>
                        <div>
    ...
    return (
        <div className="space-y-8 pb-12">
            ...
            {lowStockAlerts.length > 0 && (
                <div className="bg-amber-600 rounded-3xl p-6 text-white shadow-xl shadow-amber-100 flex items-center gap-4 animate-fade-in">
                    <div className="p-3 bg-white/20 rounded-2xl">
                        <AlertCircle size={24} />
                    </div>
                    <div>
                        <h3 className="text-lg font-black tracking-tight leading-tight">Attention Needed</h3>
                        <p className="text-amber-100 text-sm">There are {lowStockAlerts.length} products currently below their threshold.</p>
                    </div>
                </div>
            )}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 className="text-xl font-black text-gray-900 uppercase tracking-widest leading-none">Inventory Categories</h2>
                    <p className="text-gray-400 text-xs font-medium mt-2">Manage groups and stock warning thresholds</p>
                </div>
                <button 
                    onClick={handleAdd}
                    className="inline-flex items-center justify-center px-6 py-3.5 bg-indigo-600 text-white rounded-2xl text-sm font-black shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-95"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {categories.map((cat) => {
                    ...
                    return (
                        <div key={cat.id} className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden flex flex-col group hover:shadow-xl hover:border-indigo-100 transition-all">
                            ...
                            <div className="p-6 space-y-6 flex-1 flex flex-col">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">THESHRHOLD: {cat.lowStockThreshold}</p>
                                        </div>
                                    </div>
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    <div className="bg-gray-50 p-3 rounded-2xl text-center">
                                        ...
                                        <p className="text-[8px] font-bold text-gray-400 uppercase tracking-tighter">Total Items</p>
                                    </div>
                                    <div className={clsx("p-3 rounded-2xl text-center transition-colors", lowStockCount > 0 ? "bg-amber-50 text-amber-900 shadow-inner" : "bg-gray-50")}>
                                        <p className={clsx("text-xs font-black", lowStockCount > 0 ? "text-amber-600" : "text-gray-900")}>{lowStockCount}</p>
                                        <p className="text-[8px] font-bold text-gray-400 uppercase tracking-tighter">Low Stock</p>
                                    </div>
                                    <div className={clsx("p-3 rounded-2xl text-center transition-colors", outOfStockCount > 0 ? "bg-red-50 text-red-900 shadow-inner" : "bg-gray-50")}>
                                        <p className={clsx("text-xs font-black", outOfStockCount > 0 ? "text-red-600" : "text-gray-900")}>{outOfStockCount}</p>
                                        <p className="text-[8px] font-bold text-gray-400 uppercase tracking-tighter">Out of Stock</p>
                                    </div>
                                </div>
                            </div>
                        </div>
