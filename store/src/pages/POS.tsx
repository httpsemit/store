...
const POS = () => {
    ...
    const handleCompleteSale = async () => {
        ...
        try {
            ...
            setIsProcessing(false);
            showToast('error', 'Failed to complete sale. Please try again.');
        }
    };
        ...
        <div className="flex flex-col h-[calc(100vh-140px)] gap-6 pb-2">
            ...
            <div className="flex gap-6 h-full overflow-hidden">
                ...
                <div className="flex-1 flex flex-col min-w-0">
                    <div className="flex flex-col sm:flex-row gap-4 mb-6 items-center">
                        <div className="relative flex-1 group">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-3 bg-white border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500 shadow-sm transition-all text-sm font-medium"
                                value={searchTerm}
                        ...
                        <button
                            ...
                            className="p-3 bg-indigo-600 text-white rounded-2xl shadow-lg hover:bg-indigo-700 transition-all flex items-center justify-center"
                        ...
                        <select
                            className="bg-white px-6 py-3 border border-gray-200 rounded-2xl shadow-sm focus:outline-none text-sm font-bold text-gray-700 appearance-none min-w-[140px]"
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 overflow-y-auto pb-6 pr-1 custom-scrollbar">
                        {filteredProducts.map(p => {
                            ...
                            return (
                                <button
                                    ...
                                    onClick={() => addToCart(p)}
                                    className={clsx(
                                        "relative text-left bg-white border border-gray-100 rounded-3xl p-5 shadow-sm hover:shadow-xl hover:border-indigo-400 transition-all group flex flex-col items-start active:scale-95",
                                ...
                                >
                                    <div className="flex items-center gap-4 mb-4">
                                        <div
                                            className="w-12 h-12 rounded-2xl flex items-center justify-center text-white font-black text-xl shadow-lg ring-4 ring-white"
                                            style={{ backgroundColor: cat?.color || '#cbd5e1' }}
                                        >
                                            {p.name.charAt(0)}
                                        </div>
                                        <div className="min-w-0">
                                            <p className="font-black text-gray-900 text-sm truncate leading-tight group-hover:text-indigo-600 transition-colors">{p.name}</p>
                                    ...
                                    <div className="w-full mt-auto pt-4 border-t border-gray-50 flex items-center justify-between">
                                        <div className="flex flex-col">
                                            ...
                                            <p className="font-black text-lg text-gray-900 leading-none">₹{p.price}</p>
                                        </div>
                                    ...
                                    {inCart && (
                                        <div className="absolute top-2 right-2 bg-indigo-600 text-white w-7 h-7 rounded-full flex items-center justify-center text-xs font-black shadow-lg shadow-indigo-200 animate-pop">
                                            {inCart.quantity}
                                        </div>
                                    )}
                                </button>
                ...
                <div className="w-[380px] flex flex-col bg-white border border-gray-100 rounded-[32px] shadow-2xl overflow-hidden shrink-0 mt-2">
                    <div className="p-6 border-b border-gray-50 flex items-center justify-between bg-indigo-50/20">
                        <div className="flex items-center gap-3">
                            <div className="p-2 bg-indigo-600 rounded-xl text-white">
                                <ShoppingCart size={18} />
                            </div>
                            <div>
                                <h3 className="font-black text-gray-900 tracking-tight leading-none text-sm uppercase">Order Cart</h3>
                                <p className="text-[9px] font-bold text-indigo-400 uppercase tracking-widest mt-1">{cart.length} Items</p>
                            </div>
                        </div>
                        <div className="text-right">
                             <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-none">ID</p>
                             <p className="text-xs font-black text-indigo-600 mt-1">#DRAFT</p>
                        </div>
                    </div>

                    <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
                        {cart.map(item => (
                            <div key={item.productId} className="flex items-center gap-4 p-4 bg-gray-50/50 rounded-3xl border border-transparent hover:border-indigo-100 hover:bg-white transition-all group">
                                <div className="flex-1 min-w-0">
                                    <p className="font-black text-gray-900 text-sm truncate">{item.productName}</p>
                                    <div className="flex items-center gap-2 mt-1">
                                        <span className="text-[10px] font-black px-1.5 py-0.5 bg-indigo-50 text-indigo-600 rounded-md">
                                            ₹{item.unitPrice}
                                        </span>
                        ...
                        {cart.length === 0 && (
                            <div className="h-full flex flex-col items-center justify-center p-10 text-center py-20">
                                <div className="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center text-gray-200 mb-6">
                                    <Barcode size={32} />
                                </div>
                                <h4 className="font-black text-gray-400 uppercase tracking-widest text-sm">Cart is Empty</h4>
                                <p className="text-xs text-gray-300 font-medium mt-2 max-w-[200px]">Scan a barcode or select an item from the left to begin.</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="p-6 bg-gray-50 border-t border-gray-100 space-y-4">
                        <div className="space-y-2">
                            ...
                            <div className="flex justify-between items-center pt-2 border-t border-gray-200">
                                ...
                                <span className="text-2xl font-black text-indigo-600 tracking-tighter">₹{total.toLocaleString('en-IN')}</span>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                    <div className="flex gap-2 relative">
                                        <div className="relative flex-1">
                                            <input
                                                type="text"
                                                className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-xs outline-none focus:ring-2 focus:ring-indigo-500 transition-all font-bold shadow-inner"
                                            ...
                                            {showCustomerResults && customerSearch.length > 0 && (
                                                <div className="absolute bottom-full left-0 right-0 mb-2 bg-white border border-gray-100 rounded-2xl shadow-2xl z-50 max-h-48 overflow-y-auto animate-pop">
                                                    {customers.filter(c => 
                                    ...
                                    <div className="grid grid-cols-3 gap-2">
                                        {[
                                            ...
                                            <button
                                                ...
                                                className={clsx(
                                                    "flex flex-col items-center justify-center gap-1.5 py-2.5 rounded-xl border-2 text-[8px] font-black uppercase tracking-tight transition-all",
                            ...
                            <button
                                ...
                                className="w-full relative py-4 px-6 bg-indigo-600 text-white rounded-[20px] font-black shadow-2xl shadow-indigo-200 hover:bg-indigo-700 transition-all active:scale-[0.98] disabled:opacity-30 disabled:grayscale overflow-hidden flex items-center justify-center gap-3 group text-sm uppercase tracking-widest"
                            >
                                {isProcessing ? (
