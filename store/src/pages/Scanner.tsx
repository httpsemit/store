    ...
    return (
        <div className="flex flex-col h-[calc(100dvh-120px)] lg:h-[calc(100vh-140px)] gap-4 sm:gap-6 pb-2">
            ...
            <div className="flex flex-col gap-4 bg-white dark:bg-[#0a0a0a] p-4 rounded-[20px] sm:rounded-[32px] border border-gray-100 dark:border-white/10 dark:border-white/5 shadow-xl no-print">
                ...
                <div className="flex bg-gray-50 dark:bg-[#121212] p-1 sm:p-1.5 rounded-[16px] sm:rounded-[24px] border border-gray-200 dark:border-white/20 dark:border-white/10">
                    <button
                        onClick={() => setMode('manual')}
                        className={clsx(
                            "flex items-center gap-2 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all",
                            mode === 'manual' ? "bg-white dark:bg-[#0a0a0a] text-emerald-600 shadow-sm" : "text-gray-400 hover:text-gray-600 dark:text-gray-300"
                        )}
                    >
                    ...
                    <button
                        ...
                        className={clsx(
                            "flex items-center gap-2 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all",
                            mode === 'camera' ? "bg-white dark:bg-[#0a0a0a] text-emerald-600 shadow-sm" : "text-gray-400 hover:text-gray-600 dark:text-gray-300"
                        )}
                    >
            ...
            <div className="flex flex-col lg:flex-row gap-4 sm:gap-6 h-full overflow-hidden">
                ...
                <div className="flex-1 flex flex-col gap-6 min-w-0">
                    {mode === 'camera' && (
                        <div className="bg-white dark:bg-[#0a0a0a] rounded-[24px] sm:rounded-[40px] border border-gray-100 dark:border-white/10 dark:border-white/5 shadow-sm flex flex-col items-center justify-center p-4 sm:p-8 relative overflow-hidden group min-h-[300px] sm:min-h-[400px]">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 overflow-y-auto pr-1 custom-scrollbar">
                            {suggestions.map(p => (
                                <button
                                    ...
                                    className="group relative text-left p-3 sm:p-4 bg-gray-50 dark:bg-[#121212]/50 rounded-lg sm:rounded-2xl border border-transparent hover:border-emerald-100 hover:bg-white dark:bg-[#0a0a0a] hover:shadow-md transition-all truncate"
                                >
                                    <p className="text-xs font-black text-gray-900 dark:text-white truncate group-hover:text-emerald-600 transition-colors uppercase tracking-tighter">{p.name}</p>
                ...
                <div className="w-full lg:w-[360px] flex flex-col bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-white/10 dark:border-white/5 rounded-[24px] sm:rounded-[40px] shadow-2xl overflow-hidden shrink-0 mt-0 lg:mt-2 max-h-[45vh] lg:max-h-full">
                    ...
                    <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-2 custom-scrollbar">
                        {scannedItems.map((item) => (
                            <div key={item.productId} className="group bg-gray-50 dark:bg-[#121212]/50 hover:bg-white dark:bg-[#0a0a0a] p-2 sm:p-3 rounded-lg sm:rounded-2xl border border-transparent hover:border-emerald-100 transition-all">
                        ...
                        {scannedItems.length === 0 && (
                            <div className="h-full flex flex-col items-center justify-center p-6 sm:p-10 text-center py-16 sm:py-20 opacity-40">
                                <div className="w-16 h-16 sm:w-20 sm:h-20 bg-gray-50 dark:bg-[#121212] rounded-full flex items-center justify-center text-gray-300 mb-4 sm:mb-6 border border-dashed border-gray-200 dark:border-white/20 dark:border-white/10">
                                    <Scan size={24} className="sm:size-12" />
                                </div>
                                <h4 className="font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest text-xs">Waiting for Input</h4>
                    ...
                    <div className="p-4 sm:p-6 bg-gray-50 dark:bg-[#121212]/50 border-t border-gray-100 dark:border-white/10 dark:border-white/5 space-y-3 sm:space-y-4 no-print">
                        <div className="space-y-3">
                            <div className="flex items-center justify-between p-2 sm:p-3 bg-white dark:bg-[#0a0a0a] rounded-lg sm:rounded-2xl border border-gray-100 dark:border-white/10 dark:border-white/5 shadow-sm">
                                <div>
